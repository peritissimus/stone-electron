...
export class MarkdownProcessor implements IMarkdownProcessor {
  ...
  extractPlainText(markdown: string): string {
    return this.htmlToPlainText(markdown);
  }
  ...
  private preprocessMarkdown(markdown: string): string {
    ...
    result = result.replaceAll(/\[\[([^\]]{1,500})\]\]/g, (_: string, noteName: string) => {
      return `<span data-type="note-link" data-title="${this.escapeHtml(noteName)}">${this.escapeHtml(noteName)}</span>`;
    });
   ...
   */
  private simpleHtmlToMarkdown(input: string): string {
    let out = input;
    ...
    out = out.replaceAll(
      ...
      (_, title: string) => `[[${this.decodeHtmlEntities(title)}]]`,
    );
    ...
    for (let i = 6; i >= 1; i--) {
      ...
      out = out.replaceAll(
        re,
        (_, text: string) => `\n\n${'#'.repeat(i)} ${this.stripTags(text).trim()}\n\n`,
      );
    }
    ...
    out = out.replaceAll(
      ...
      (_, state: string, content: string) => {
        ...
        const textContent = content
          ...
          .trim();
        // Save as list-style task item with dash prefix for consistency
        return `- ${stateLabel} ${this.stripTags(textContent)}\n`;
      },
    );
    ...
    out = out.replaceAll(/<ul[^>]*>([\s\S]*?)<\/ul>/gi, (_, inner: string) => {
      const items = inner
        ...
        .filter(Boolean);
      const formatted = items.map((it: string) => '- ' + this.stripTags(it));
      return '\n\n' + formatted.join('\n') + '\n\n';
    });
    ...
    out = out.replaceAll(
      ...
      (_, href: string, text: string) => `[${this.stripTags(text)}](${href})`,
    );

    // Inline code
    out = out.replaceAll(
      /<code[^>]*>([\s\S]*?)<\/code>/gi,
      (_, code: string) => `\`${this.stripTags(this.decodeHtmlEntities(code))}\``,
    );
