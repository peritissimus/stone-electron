...
import { useFileTreeStore, FileTreeNode } from '@renderer/services/workspace/model/fileTreeStore';
...
export function useFileTreeAPI() {
  ...
  const setCounts = useFileTreeStore((state) => state.setCounts);
  const expandAll = useFileTreeStore((state) => state.expandAll);
