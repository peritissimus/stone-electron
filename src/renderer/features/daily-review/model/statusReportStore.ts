...
export const useStatusReportStore = create<StatusReportState>((set, get) => ({
  ...
  sendToJournal: async () => {
    ...
    try {
      ...
      set({
        sending: false,
        error: err instanceof Error ? err.message : 'Failed to send to journal',
      });
      return null;
    }
  },
...
async function runGenerate(set: (state: Partial<StatusReportState>) => void): Promise<void> {
  try {
...
function formatJournalBody(result: StatusReportResult): string {
  return [
    `### Weekly status (${result.windowStart} → ${result.windowEnd})`,
    '',
    result.report.trim(),
  ].join('\n');
}
