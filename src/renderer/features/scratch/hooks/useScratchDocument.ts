...
export function useScratchDocument(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Commit the new file identity before parsing a potentially large
        // document. Yielding one frame gives the desktop shell a chance to
        // paint immediate navigation feedback instead of leaving the previous
        // filename on screen throughout a long Markdown parse.
