...
import {
  useMeetingRecorder,
  type LiveLine,
} from '@renderer/features/meetings/hooks/useMeetingRecorder';
import { useSystemAudioPermission } from '@renderer/features/onboarding/hooks/useOnboarding';
  ...
  return (
    ...
    >
      {phase === 'recording' ? (
        ...
        <Status
          icon={<CircleNotch size={18} className="animate-spin text-primary" />}
          title="Preparing…"
          detail="Reserving slot and asking for the microphone."
        />
      ) : phase === 'uploading' ? (
        <Status
          icon={<CircleNotch size={18} className="animate-spin text-primary" />}
          title="Saving audio"
          detail="Storing the recording locally."
        />
      ) : phase === 'finalizing' ? (
        <Status
          icon={<CircleNotch size={18} className="animate-spin text-primary" />}
          title="Transcribing & summarising"
          detail="Runs on this device. The speech model downloads once on first use."
        />
      ) : phase === 'done' ? (
        <Status
          icon={
