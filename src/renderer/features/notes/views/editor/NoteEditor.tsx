...
import { useEditorOperations } from '@renderer/features/notes/hooks/useNoteEditor';
import { useNavigateToNote, useNavigateHome } from '@renderer/services/navigation';
import { useNoteAPI } from '@renderer/features/notes/commands/useNoteAPI';
import { useRichTextEditor, type RichTextEditor } from '@renderer/features/notes/editor';
import { setEditorDocument } from '@renderer/features/notes/editor/document';
import { useDocumentBuffer } from '@renderer/services/documents/hooks/useDocumentBuffer';
import { useImageUpload } from '@renderer/features/notes/hooks/useImageUpload';
import { useEditorMode } from '@renderer/features/notes/hooks/useEditorMode';
import { useNoteExport } from '@renderer/features/notes/hooks/useNoteExport';
import { useAutosave } from '@renderer/features/notes/hooks/useAutosave';
...
export const NoteEditor = forwardRef<NoteEditorHandle, NoteEditorProps>(function NoteEditor(
  ...
  const { activeNote, activeNoteId, activeNoteFilePath, activeWorkspace, removeBuffer } =
    useEditorOperations();
  ...
  useEffect(() => {
    ...
    const prevScrollContainer =
      prevMode === 'rich' ? richEditorScrollRef.current : rawEditorScrollRef.current;
    if (
      prevScrollContainer &&
      prevScrollContainer.scrollHeight > prevScrollContainer.clientHeight
    ) {
      const scrollableHeight = prevScrollContainer.scrollHeight - prevScrollContainer.clientHeight;
      scrollPercentRef.current =
        scrollableHeight > 0 ? prevScrollContainer.scrollTop / scrollableHeight : 0;
    }

    // Restore scroll position in new editor after a brief delay (for render)
    requestAnimationFrame(() => {
      const newScrollContainer =
        editorMode === 'rich' ? richEditorScrollRef.current : rawEditorScrollRef.current;
