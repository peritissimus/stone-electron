...
import { useRelatedNotes } from '@renderer/features/notes/hooks/useRelatedNotes';
import { useForwardLinks } from '@renderer/features/notes/hooks/useForwardLinks';
...
export function RelatedNotesPanel({ noteId, editor }: RelatedNotesPanelProps) {
  ...
  const visibleResults = useMemo(() => results.filter((r) => r.similarity > 0.4), [results]);
          ...
          <div className="relative size-3.5">
            <div
              className={cn(
                ...
                isExpanded ? 'scale-100 opacity-100 blur-0' : 'scale-[0.25] opacity-0 blur-[4px]',
              )}
            >
            ...
            <div
              className={cn(
                ...
                'transition-[opacity,filter,scale] duration-300',
                '[transition-timing-function:cubic-bezier(0.2,0,0,1)]',
                isExpanded ? 'scale-[0.25] opacity-0 blur-[4px]' : 'scale-100 opacity-100 blur-0',
              )}
            >
      ...
      {isExpanded && (
        <div className="mx-auto max-w-3xl space-y-1 px-4 pb-3">
          {loading && visibleResults.length === 0 ? (
            ...
            <div className="px-3 py-2 text-xs text-muted-foreground">No related notes found.</div>
          ) : (
...
function RelatedRow({ match, linked, canLink, onClick, onLink }: RelatedRowProps) {
  const heading =
    match.bestChunk.headingPath.length > 0 ? match.bestChunk.headingPath.join(' › ') : null;
  ...
  return (
    <li className="group relative">
      ...
      >
        ...
        {heading && <div className="mt-0.5 truncate text-xs text-muted-foreground">{heading}</div>}
