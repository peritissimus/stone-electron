...
import { loadLanguage } from '@renderer/features/notes/editor/config/codeLanguages';
...
const CodeBlockComponentView: React.FC<CodeBlockComponentProps> = ({
  node,
  updateAttributes,
  ...
  const isLargeDocument = editor?.state.doc.content.size > 750_000;

  // Listen for theme changes
  useEffect(() => {
    if (isLargeDocument && !isDiagram) return;
    const observer = new MutationObserver(() => {
      setIsDarkMode(document.documentElement.classList.contains('dark'));
    });
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });
    return () => observer.disconnect();
  }, [isDiagram, isLargeDocument]);
  ...
  // Thousands of interactive toolbars and theme observers make very large
  // technical documents expensive to lay out after every edit. Keep regular
  // blocks lightweight until selected; diagrams retain their interactive UI.
  if (isLargeDocument && !isDiagram && !selected) {
    return (
      <NodeViewWrapper className="code-block-wrapper" data-language={language}>
        <pre className="my-4 overflow-x-auto rounded-lg border border-border bg-code-bg">
...
// ProseMirror emits an editor transaction for every document edit. Most code
// block node views are untouched by a transaction elsewhere in the document;
// avoid re-rendering every toolbar/highlighter when their node and selection
// are unchanged. This is especially important for large technical documents.
export const CodeBlockComponent = React.memo(
  CodeBlockComponentView,
  (previous, next) =>
