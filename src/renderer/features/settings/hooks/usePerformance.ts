...
import { performanceAPI } from '@renderer/api';
import type {
  PerformanceSnapshot,
  StartupMetrics,
  MemoryMetrics,
  ...
  IPCMetrics,
  DatabaseMetrics,
} from '@renderer/api/performanceAPI';
import {
  startRendererMonitoring,
  stopRendererMonitoring,
  getRendererPerformance,
  getWebVitals,
} from '@renderer/services/telemetry/performance';
...
export function usePerformance(options: UsePerformanceOptions = {}) {
  const { autoStart = false, pollInterval = DEFAULT_POLL_INTERVAL, sinceMs } = options;
