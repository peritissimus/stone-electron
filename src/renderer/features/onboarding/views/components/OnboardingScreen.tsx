...
import { useQuickCaptureShortcut } from '@renderer/features/quick-capture/hooks/useQuickCaptureShortcut';
    ...
    <div className="flex h-full w-full justify-center overflow-y-auto bg-background px-6 py-14">
      <div className="w-full max-w-md">
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-12px_rgba(0,0,0,0.12)]">
          ...
          <div key={step} className="animate-in fade-in" style={{ animationDuration: '200ms' }}>
            {step === 'workspace' && (
              <WorkspaceStep
                path={path}
                name={name}
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <Caption className="text-muted-foreground">
              Sends notes to your provider. Off = fully local.
            </Caption>
          </div>
        </div>
        <Switch
      ...
      {conflict && !recording && (
        <div className="flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
          ...
          <Body size="sm" className="text-destructive">
            {shortcut
              ? `${formatAccelerator(shortcut)} is already in use by another app.`
